...
const assistantInput=document.querySelector('#assistantQuestion');
const assistantButton=document.querySelector('#assistantAsk');
const assistantOutput=document.createElement('div');
assistantOutput.className='result';assistantOutput.id='assistantResult';assistantOutput.setAttribute('aria-live','polite');assistantOutput.hidden=true;
...
const askTheory=async()=>{
  const question=assistantInput?.value.trim();
  if(!question||question.length<12){assistantOutput.hidden=false;assistantOutput.textContent='Please provide a focused theory question of at least 12 characters.';return;}
  if(!assistantConfigured){assistantOutput.hidden=false;assistantOutput.textContent='The assistant is not configured yet.';return;}
  ...
  try{
    ...
    assistantOutput.textContent=body.answer;
  }catch(error){
    assistantOutput.textContent=`The assistant is unavailable: ${error.message}`;
  }finally{assistantButton.disabled=false;}
};

if(assistantButton)assistantButton.onclick=askTheory;
assistantForm?.addEventListener('submit',event=>{event.preventDefault();askTheory();});
