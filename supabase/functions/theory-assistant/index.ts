...
Deno.serve(async(req)=>{
  ...
  const {question}=await req.json().catch(()=>({question:''}));if(typeof question!=='string'||question.trim().length<12||question.length>1500)return respond({error:'Question must contain 12–1500 characters.'},400);
  ...
  const since=new Date(Date.now()-86400000).toISOString();const {count}=await service.from('ai_usage').select('*',{count:'exact',head:true}).eq('client_hash',digest).gte('created_at',since);if((count||0)>=5)return respond({error:'Free daily allowance reached. Please return tomorrow.'},429);
  const key=Deno.env.get('OPENROUTER_API_KEY');if(!key)return respond({error:'AI provider is not configured.'},503);
  ...
  if(!upstream.ok)return respond({error:'The free AI provider is temporarily unavailable.'},503);const data=await upstream.json();const answer=data.choices?.[0]?.message?.content;if(!answer)return respond({error:'No answer was returned.'},503);
  await service.from('ai_usage').insert({client_hash:digest,model:data.model||'openrouter/free'});return respond({answer,model:data.model||'openrouter/free'});
});
