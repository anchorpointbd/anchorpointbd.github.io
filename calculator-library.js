...
const select=document.querySelector('#libraryDepartment'),search=document.querySelector('#calculatorSearch'),more=document.querySelector('#calculatorMore'),count=document.querySelector('#calculatorResultCount');let limit=6;
function filter(){const dept=select?.value||'all',term=(search?.value||'').trim().toLowerCase();const matched=[...root.children].filter(card=>(dept==='all'||card.dataset.libraryDepartment===dept)&&(!term||card.dataset.librarySearch.includes(term)));[...root.children].forEach(card=>card.hidden=true);matched.slice(0,limit).forEach(card=>card.hidden=false);if(count)count.textContent=matched.length?`Showing ${Math.min(limit,matched.length)} of ${matched.length} matching calculators`:'No close calculator match—try a quantity, unit, equipment type or discipline.';if(more){more.hidden=matched.length<=limit;more.textContent=`Show ${Math.min(6,matched.length-limit)} more`}}
select?.addEventListener('change',()=>{limit=6;filter()});search?.addEventListener('input',()=>{limit=6;filter()});more?.addEventListener('click',()=>{limit+=6;filter()});filter();window.anchorPointCalculatorCount=defs.length;
})();