...
root.innerHTML=defs.map((d,i)=>`<article class="library-calculator" data-library-department="${d[0]}" data-library-search="${esc([d[1],d[2],labels[d[0]],...d[3].flat()].join(' ').toLowerCase())}"><p class="tag">${String(i+1).padStart(2,'0')} · ${esc(labels[d[0]])}</p><h3>${esc(d[1])}</h3><div class="library-fields">${d[3].map(x=>`<label>${esc(x[1])} <small>${esc(x[2])}</small><input type="number" step="any" data-key="${x[0]}" value="${x[3]}"></label>`).join('')}</div><button type="button" class="pill ink library-calc">Calculate</button><details><summary>Formula and boundary</summary><p class="formula">${esc(d[2])}</p><p>Screening calculation only. Verify inputs, units, operating case, applicable standards and discipline-specific limits before consequential use.</p></details><p class="result" aria-live="polite">Enter values and calculate.</p></article>`).join('');
root.addEventListener('click',e=>{const b=e.target.closest('.library-calc');if(!b)return;const card=b.closest('.library-calculator'),i=[...root.children].indexOf(card),d=defs[i],v={};let ok=true;card.querySelectorAll('input').forEach(x=>{v[x.dataset.key]=Number(x.value);if(!Number.isFinite(v[x.dataset.key]))ok=false});const out=card.querySelector('.result');try{if(!ok)throw Error();const [value,unit]=d[4](v);if(!Number.isFinite(value))throw Error();out.textContent=F(value,unit);card.classList.remove('input-error')}catch{out.textContent='Check the inputs. Divisors, efficiencies, lengths and rates must be greater than zero.';card.classList.add('input-error')}});
const select=document.querySelector('#libraryDepartment'),search=document.querySelector('#calculatorSearch'),more=document.querySelector('#calculatorMore'),count=document.querySelector('#calculatorResultCount');let activeIndex=0;
function filter(){const dept=select?.value||'all',term=(search?.value||'').trim().toLowerCase();const matched=[...root.children].filter(card=>(dept==='all'||card.dataset.libraryDepartment===dept)&&(!term||card.dataset.librarySearch.includes(term)));[...root.children].forEach(card=>card.hidden=true);if(matched.length){activeIndex=Math.min(activeIndex,matched.length-1);matched[activeIndex].hidden=false}if(count)count.textContent=matched.length?`Tool ${activeIndex+1} of ${matched.length} matching calculators · one focused workspace`:'No close calculator match—try a quantity, unit, equipment type or discipline.';if(more){more.hidden=matched.length<=1;more.textContent='Next matching calculator →';more.dataset.matches=matched.length}}
select?.addEventListener('change',()=>{activeIndex=0;filter()});search?.addEventListener('input',()=>{activeIndex=0;filter()});more?.addEventListener('click',()=>{const total=Number(more.dataset.matches)||1;activeIndex=(activeIndex+1)%total;filter();root.scrollIntoView({behavior:'smooth',block:'center'})});filter();window.anchorPointCalculatorCount=defs.length;
})();
